...
function trendingCardViews(card){
    if(!card) return 0;
    // V173: Trending uses one shared catalogue metric for every visitor.
    // Do not mix owner-only qualified-view maps with public card data, because
    // that can produce different rankings between sessions/devices.
    return Math.max(0,Number(card.view_count||0));
  }

function trendingCardScore(card){
    // Score is intentionally just the shared view count. Freshness is applied
    // only as a tie-break below, so a zero-view card can never become Trending.
...
function getFiltered(){
    ...
    if(appContext.activeQuickFilter === "trending"){
      // A listing must have genuine shared catalogue activity to qualify.
      // This prevents newly-added / zero-view cards from filling empty slots.
      ...
      // Keep the highest-viewed matching listings. Do not backfill with cards
      // that have no activity just to reach the discovery limit.
  ...
  Object.assign(appContext,{isNewCard,cardLifecycle,isLiveLifecycle,cardMatchesListingScope,listingScopeMeta,normalizeFilterValue,titleCaseWords,giveawayDisplayTitle,normalizeStoredLabel,canonicalAvailability,selectedSetMatches,effectiveFormat,isChampionshipSeries,cardGradeSortScore,trendingCardViews,trendingCardScore,compareNullableNumber,getFiltered});
}
